...
- Corrected at integration (2026-09-23): the deployed ranker artifact (sha256 `eba2bcb6…`) reads its own stored feature list, which matches `config.json` `ml_ranker.features` and contains **no** `team_margin`, `total_line` or `spread_line` (checked by loading the artifact; `NUMERIC_FEATURES` in code still lists them for the full feature space, but inference uses the artifact's list). It does read the offered threshold (`line`, `z`, `mean_minus_line`, `sd_over_line`) and no price or consensus. It was trained when `game_script` was spread-derived; at inference `game_script` is now neutral (1.0). No retrain or revalidation is claimed. Cards label ranker inputs as ordering-only from the run receipt (`ordering_features_populated`).
